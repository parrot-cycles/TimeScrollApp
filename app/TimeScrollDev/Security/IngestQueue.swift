...
final class IngestQueue {
    ...
    private var dir: URL {
        ...
        return base.appendingPathComponent("TimeScroll/IngestQueue", isDirectory: true)
    }
    ...
    private func drainOnce() {
        ...
        for u in recs.prefix(200) {
            do {
                let blob = try Data(contentsOf: u)
                ...
                if rowId > 0 { try fm.removeItem(at: u); processed += 1 }
            } catch {
                // Move to failed/
        ...
        let remaining = ((try? fm.contentsOfDirectory(at: dir, includingPropertiesForKeys: nil)) ?? []).filter { $0.pathExtension == "iq1" }.count
